...
Adapted from:
    https://github.com/nirajdevpandey/face-emotion-detection
...
class GenderEmotionDetector:
    """A class that contains all steps to handel a CNN model for emotion and gender detection.

    This Class download the pre-trained weights, load the model, eventually re-train the model
    and predict people emotions and gender giving a picture.

    Attributes:
        face_detection (cv.CascadeClassifier): pre_trained haarcasade face detector.
        emotion_classifier (Keras.model): CNN model for emotion classification.
        gender_classifier (Keras.model): CNN model for gender classification.
        emotion_labels (list): list of emotion classes.
    """
    ...
    def __init__(self, detection_model_path=PATH + "people_emotions/people_emotions/gender_emotion_detector/trained model/haarcascade_frontalface_default.xml",
        ...
        """load the face detector cascade, emotion detection CNN and gender dection CNN
        then define the list of emotion labels.

        Args:
            emotion_model_path (str): path of the CNN emotion detection model .
            gender_model_path (str): path of the CNN gender detection model.
            detection_model_path (str): path of the haarcascade xml file.
        """
        # loading models
    ...
    def predict(self, image_path, results_dir=PATH + "people_emotions/people_emotions/gender_emotion_detector/results"):
        """predict people emotions and gender given an image.

        detect people faces using harrcascade then classify the emotion of the face and his gender. This
